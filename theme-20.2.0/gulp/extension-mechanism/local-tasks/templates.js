...
function generateLibraryFile(cb)
{
	...
	if(shell.test('-f', path.join(configurations.folders.output, outputFile)))
	{
		cb();
		return;
	}
	...
	gfile(indexFile.name + '.js', '', {src: true})
		...
		.pipe(gulp.dest(configurations.folders.output, { mode: '0777' }))
		.on('end', cb);
}
...
function runTemplates(gulpDone)
{
	...
	async.each(manifest_manager.getTemplateApplications(), (application, cb)=>
	{
		...
		gulp.src(_.union([path.join(configurations.folders.output, 'javascript-libs.js')], templates), {allowEmpty: true})
