	...
	<div class="cart-lines-table-middle">
		...
		{{#if isNavigable}}
			<a {{{linkAttributes}}} class="cart-lines-name-link">
				{{item.custitem_long_title}}
			</a>
		{{else}}
				<span class="cart-lines-name-viewonly">{{item.custitem_long_title}}</span>
		{{/if}}
		</div>
		<div class="cart-line-format">
			{{item.custitem_webstore_format}}
		</div>
		...
		<p class="cart-line-digital-access">Access to digital products is available immediately after completing your purchase. Please visit
			<a href="https://eproducts.westacademic.com/" target="_blank">eproducts.westacademic.com</a> and sign in.
		</p>
		{{/if}}

		<!-- POD Message -->
		{{#if item.custitem_print_on_demand_confirm}}
		<p class="cart-line-digital-access">To purchase the spiralbound Print on Demand version of this book you must purchase the eBook version in this same transaction or have previously purchased the eBook version.  You may only purchase one Print on Demand version of this title. 
					<br><br>
