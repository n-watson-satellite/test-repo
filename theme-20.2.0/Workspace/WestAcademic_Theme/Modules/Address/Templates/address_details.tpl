...
{{else}}
	{{#if showSelector}}
		<a class="address-details-selector {{#if isSelected}}address-details-selected{{/if}}" data-action="select" data-id="{{internalid}}">
			<input type="radio" name="address-options-{{manageOption}}" class="address-details-selector-option" id="{{internalid}}"  data-id="{{internalid}}" value="{{internalid}}" {{#if isSelected}} checked {{/if}}>
			{{#if isSelected}}
				<label for="{{internalid}}"><b>{{translate 'Selected'}}</b></label>
			{{else}}
				<label for="{{internalid}}">{{translate 'Select'}}</label>
			{{/if}}
		</a>
	{{/if}}
